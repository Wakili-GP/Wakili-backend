...
public class GetAllSystemReviewsQueryHandler(ISystemReviewRepository systemReviewRepository)
    : IRequestHandler<GetAllSystemReviewsQuery, Result<List<SystemReviewResponseDto>>>
{
    ...
    {
        var reviews = await systemReviewRepository.GetAllAsync(cancellationToken);
